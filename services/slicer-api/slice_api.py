...
app = FastAPI(title="slicer-api", version="0.4.1")
...
async def get_spools():
    ...
    for s in spools:
        ...
        mat = (fil["material"].get("name") if isinstance(fil.get("material"), dict) else fil.get("material")) \
              or s.get("material") or "N/D"
        color_name = fil.get("color_name") or fil.get("colour_name") \
                     or (s.get("color") or {}).get("name") \
                     or s.get("color_name") or s.get("name") or "—"
        color_hex  = fil.get("color_hex") or fil.get("colour_hex") \
                     or (s.get("color") or {}).get("hex") \
                     or s.get("color_hex") or "#777777"
        price_per_kg = s.get("price_per_kg") or s.get("cost_per_kg") \
...
# Alias di retrocompatibilità per la UI vecchia
@app.get("/inventory")
async def inventory_alias():
    return await get_spools()

# Slicing di esempio (immutato)
